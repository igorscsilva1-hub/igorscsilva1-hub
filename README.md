# Igor Santana do Couto Silva

Estudante de Tecnologia em Segurança da Informação na FATEC São Caetano do Sul, com formação técnica em Redes de Computadores pela ETEC de Embu.

Meu objetivo é seguir carreira em segurança ofensiva (Pentest/Red Team), e estou construindo base prática para isso.

## Foco atual
- Segurança Ofensiva e Pentest
- Redes e Infraestrutura
- Linux

## Conhecimentos
- Redes TCP/IP e infraestrutura
- Linux (terminal, permissões, sudo, apt)
- Virtualização (VirtualBox)
- Noções de segurança ofensiva, simulação de ataques em ambiente controlado com Kali Linux
- Conceitos de Segurança da Informação e LGPD
- Python (básico)

## Formação
- Tecnologia em Segurança da Informação — FATEC São Caetano do Sul (em andamento)
- Técnico em Redes de Computadores — ETEC de Embu (2025)

## Certificações
- Prevenção e Reação a Incidentes Cibernéticos - SENAI (2026)
- Introdução ao Hacking Ético e Pentest — Solyd Offensive Security (2025)
- Lei Geral de Proteção de Dados — Fundação Bradesco (2025)
- Introdução à Cibersegurança - Cisco Networking Academy (2026)
- Analista SOC Essencial - IBSEC (2026)
- Fundamentos de Tecnologia da Informação - IBM SkillsBuild (2026)


## Contato
- LinkedIn: [linkedin.com/in/igorscsilva](https://linkedin.com/in/igorscsilva)
- E-mail: igorscsilva1@gmail.com

<!--
**igorscsilva1-hub/igorscsilva1-hub** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
